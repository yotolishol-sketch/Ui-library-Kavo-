...
function Kavo:DraggingEnabled(frame, parent)
    ...
    -- ใช้ frame.InputBegan แทน global เพื่อป้องกัน input ไปถึง Roblox camera
    frame.InputBegan:Connect(function(inp, gameProcessed)
    ...
    -- ใช้ frame-level InputChanged เพื่อไม่ให้ camera รับ input ด้วย
    ...
    -- หยุด drag ทันทีที่ปล่อยนิ้ว/เมาส์
...
function Kavo.CreateLib(kavName, themeList)
    ...
    -- ปุ่ม Minimize (−/+) อยู่ซ้ายปุ่มปิด
    local minimize = Instance.new("ImageButton")
    minimize.Name = "minimize"
    minimize.Parent = MainHeader
    minimize.BackgroundTransparency = 1.000
    minimize.Position = UDim2.new(0.906, 0, 0.137999997, 0)
    minimize.Size = UDim2.new(0, 21, 0, 21)
    minimize.ZIndex = 2
    minimize.Image = "rbxassetid://3926305904"
    minimize.ImageRectOffset = Vector2.new(284, 84)  -- ไอคอน minus
    minimize.ImageRectSize = Vector2.new(24, 24)
    minimize.ImageColor3 = Color3.fromRGB(255, 255, 255)

    local minimized = false
    local fullSize = UDim2.new(0, 525, 0, 318)
    local miniSize = UDim2.new(0, 525, 0, 29)  -- เหลือแค่ header

    minimize.MouseButton1Click:Connect(function()
        if not minimized then
            -- ย่อ: แสดงแค่ header
            minimized = true
            minimize.ImageRectOffset = Vector2.new(284, 124)  -- ไอคอน plus
            Main.ClipsDescendants = true
    ...
    function Tabs:NewTab(tabName)
        ...
        coroutine.wrap(function()
            ...
            function Elements:NewSlider(slidInf, slidTip, maxvalue, minvalue, callback)
                ...
                -- ฟังก์ชันกลางสำหรับอัพเดต slider ด้วย X position
                local function updateSlider(posX)
                    sliderDrag:TweenSize(UDim2.new(0, math.clamp(posX - sliderDrag.AbsolutePosition.X, 0, 149), 0, 6), "InOut", "Linear", 0.05, true)
                    Value = math.floor((((tonumber(maxvalue) - tonumber(minvalue)) / 149) * math.clamp(posX - sliderDrag.AbsolutePosition.X, 0, 149)) + tonumber(minvalue))
                    val.Text = Value
                ...
                local function stopSlider()
                    if sliding then
                        ...
                        if touchmoveconn then touchmoveconn:Disconnect() end
                    ...
                    if not focusing then
                        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
                            ...
                            if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                                ...
                                releaseconnection = uis.InputEnded:Connect(function(endInp)
                                    if endInp.UserInputType == Enum.UserInputType.MouseButton1 then
                                        stopSlider()
                                    end
                                end)
                            else
                                -- Mobile: ติดตาม Touch input นี้เท่านั้น ป้องกัน slide ค้าง
                                touchmoveconn = inp.Changed:Connect(function()
                                ...
                                touchendconn = uis.InputEnded:Connect(function(endInp)
                                    if endInp.UserInputType == Enum.UserInputType.Touch then
                                        stopSlider()
                                    end
                                end)
                            end
